...
class sgnDict(collections.UserDict, dict):
    ...
    def __init__(self, *args, name=None, create=None, buffer_size=10_000, serializer=pickle, shared_lock=None, full_dump_size=None,
        ...
        if shared_lock:
            try:
                ...
                try:
                    self.lock = ILock(name=self.name,reentrant=True);
                except NameError:
                    #self.cleanup()
                    raise Exceptions.MissingDependency("Install `atomics` or `ilock` Python package to use shared_lock=True") from None
        else:
            self.lock = multiprocessing.RLock()

        self.shared_lock = shared_lock

        # Parameters that could be read from remote if we are connecting to an existing sgnDict
        self.recurse = recurse

        # In recurse mode, we must ensure a recurse register
        if self.recurse:
            # Must be either the name of an sgnDict as a string or an sgnDict instance
            if recurse_register is not None:
                if type(recurse_register) == str:
                    self.recurse_register = sgnDict(name=recurse_register)
                elif type(recurse_register) == sgnDict:
            ...
            # If no register was defined, we should create one
                ...
                self.recurse_register = sgnDict(name=f'{self.name}_register',
    ...
    def __setitem__(self, key, item):
        ...
        with self.lock:
            ...
            if self.recurse:
                ...
                assert type(self.recurse_register) == sgnDict, "recurse_register must be an sgnDict instance"
    ...
    def raise_already_closed(self, *args, **kwargs):
        raise Exceptions.AlreadyClosed('sgnDict already closed, you can only access the `sgnDict.data` buffer!')
    ...
    def unlink_recursed(self):
        ...
        if not self.recurse or (type(self.recurse_register) != sgnDict):
            raise Exception("Cannot unlink recursed for non-recurse sgnDict")
    ...
    def unlink_by_name(name, ignore_errors=False):
        ...
        try:
            ...
            memory = sgnDict.get_memory(create=False, name=name)
            memory.unlink()
            memory.close()
            return True
        except Exceptions.CannotAttachSharedMemory as e:
