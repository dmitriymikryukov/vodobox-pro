...
class IPClist(MutableSequence):
	...
	def __init__(self, initlist=None, path=None):
		try:
			...
			super().__init__()
			self.data=[]
			#IPCdict.connection.dict[self.path]=self.data			
	...
	def __del__(self):
		del IPCdict.paths[self.path]

	def __repr__(self):
		self.data=IPCdict.connection.dict[self.path]
	...
	def __getitem__(self, idx):
		...
		try:
			...
			raise IndexError("Index %s is not found in %s"%(idx,self.path))
		if isinstance(v,dict):
			key='%s%s%s'%(self.path,IPCdict.separator,idx)	
			v=IPCdict(path=key)
	...
	def pop(self, idx=-1):
		...
		while idx<(len(self)+1):
			...
			for k in IPCdict.connection.dict.keys():
				if k.startswith(key):
					okey='%s%s%s%s'%(self.path,IPCdict.separator,idx-1,k[len(key):])
...
class IPCdict(dict):
	...
	def __new__(class_,*args,**kwargs):
		...
		if 'path' in kwargs:
			...
			path=class_.separator
		if class_.connection is None:
			class_.connection=IPCdictConnection(is_server=is_server)
		if not (path in class_.paths):
			class_.paths[path]=super().__new__(class_,*args,**kwargs)
		return class_.paths[path]
	...
	def __init__(self,*args,**kwargs):
		try:
			...
			if 'path' in kwargs:
				...
				self.path=IPCdict.separator
			self.dict=IPCdict.connection.dict
			super().__init__(self,*args,**kwargs)

	def __del__(self):
		del IPCdict.paths[self.path]
		try:
			del IPCdict.connection.dict[self.path]
		except:
			pass
		if IPCdict.connection and IPCdict.connection.is_server and self.path==IPCdict.separator:
	...
	def __setitem__(self, key, item):
		_key=key
		key=self._cokey(key)
		key='%s%s%s'%(self.path,IPCdict.separator,key)
	...
	def keys(self):
		res=[]
		pt="%s%s"%(self.path,IPCdict.separator)
		sl=len(pt)
